...
La clave se deriva de una contraseña usando **PBKDF2HMAC** con SHA-256 y 100.000 iteraciones. Como limitación didáctica intencional, el salt es fijo — en producción el salt debe ser aleatorio y almacenarse junto al archivo cifrado.

### Alcance del Cifrado

El `TARGET` define el alcance del cifrado. Durante el desarrollo se probaron distintas configuraciones, desde una carpeta de pruebas específica hasta `C:\` completo con una lista de exclusiones para evitar dejar el sistema completamente inutilizable durante las pruebas.
...
La pantalla de rescate ocupa todo el escritorio del usuario y muestra un contador regresivo, las instrucciones de pago y el campo para ingresar la clave de descifrado. Se implementó con **tkinter**.
